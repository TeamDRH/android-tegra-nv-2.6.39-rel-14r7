...
static struct platform_device * tegra_usb_otg_host_register(void)
{
	...
	//gpio_direction_input(SMBA9701_USB0_VBUS);
...
static void tegra_usb_otg_host_unregister(struct platform_device *pdev)
{
	...
	/* Disable VBUS power. This means that if a USB host
	   ...
	   mode */
	//gpio_direction_output(SMBA9701_USB0_VBUS, 0); 
...
int __init smba_usb_register_devices(void)
{
  ...
  writel(1 << CLR_USBD_RST, (u32)rst_device_reg + CLK_RST_CONTROLLER_RST_DEV_L_CLR_0);
	...
	gpio_request(SMBA9701_USB0_VBUS, "USB0 VBUS"); /* VBUS switch, perhaps ? -- Tied to what? -- should require +5v ... */
	
	/* 0 = Gadget */
	gpio_direction_output(SMBA9701_USB0_VBUS, 0 ); /* Gadget */
